...
function generateFrames($canvas, count = 32) {
  ...
  return imageDatas.map((data) => {
    ...
    $c.getContext("2d", { willReadFrequently: true }).putImageData(data, 0, 0);
    return $c;
  });
}
 ...
 */
export function disintegrateElement($elm) {
  const $config = {
    backgroundColor: null,
  };
  html2canvas($elm, $config).then(($canvas) => {
    ...
    if (!DEBUG) {
      ...
      $frames.forEach(($frame) => {
        ...
        $frame.style.transform = `rotate(${15 * (Math.random() - 0.5)}deg) translate(${
          60 * Math.cos(randomRadian)
        }px, ${30 * Math.sin(randomRadian)}px)
...
export function bindDisintegration() {
